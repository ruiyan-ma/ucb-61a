    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.can_contain(self):
                ...
                # place.ant should be the container ant!
                place.ant = self
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        distance = 0
        # distance >= min_range
        while distance < self.min_range and target is not beehive:
            target = target.entrance
            distance += 1
        # distance <= max_range
        while distance <= self.max_range \
                and target is not beehive and not target.bees:
            target = target.entrance
            distance += 1
        # target is beehive or distance > max_range, return None
        if target is beehive or distance > self.max_range:
            return None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # if FireAnt's armor drops to 0, do damage to all the bees by its damage
        if amount >= self.armor:
            reflected_damage += self.damage
        # damaging a bee may cause it to be removed from its place,
        # but in for loop you can't remove element from the list
...
# The WallAnt class
class WallAnt(Ant):
    """A WallAnt has a large armor value. """
    name = "Wall"
    food_cost = 4
    implemented = True
        ...
        if self.true_queen:
            ...
            while place is not None:
                ...
                if isinstance(ant, ContainerAnt) \
                        ...
                        and not ant.contained_ant.damage_doubled:
                    ant.contained_ant.damage *= 2
                    ant.contained_ant.damage_doubled = True
                place = place.exit
        else:
