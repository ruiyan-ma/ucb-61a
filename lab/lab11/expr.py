...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        Number(14)
        """
        # self.operator is an instance of Expr
        operator = self.operator.eval(env)
        if operator is None:
            raise NameError('{0} is not defined.'.format(self.operator))
        # self.operands is a list of Expr instances.
        operands = [operand.eval(env) for operand in self.operands]
        # Value.apply(self, attributes) takes a list of Value instances as attributes
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        # create new local frame (local environment)
        local_env = self.parent.copy()
        for param, arg in zip(self.parameters, arguments):
            local_env[param] = arg
        # evaluate the body using the newly created frame
